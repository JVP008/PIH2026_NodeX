                            ...
                            contractor.service === 'Electrical' ? '⚡' :
                                contractor.service === 'Cleaning' ? '🧹' :
                                    contractor.service === 'Painting' ? '👨‍🎨' :
                                        contractor.service === 'HVAC' ? '❄️' :
                                            contractor.service === 'Landscaping' ? '🌳' : '👷‍♂️'
                    )}
                </div>
                <div className="flex-1 min-w-0 flex flex-col h-full">
                    <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0 flex-1">
                            ...
                            <p className="text-black font-medium text-sm mt-1 truncate">{contractor.service} • {contractor.location}</p>
                        </div>
                        <div className="text-right shrink-0">
                            ...
                            <p className="text-sm text-black font-bold whitespace-nowrap">{contractor.completed_jobs} jobs</p>
                        </div>
                    </div>

                    <p className="text-gray-800 mt-3 font-medium leading-relaxed break-words line-clamp-3">
                        {contractor.description}
                    </p>

                    <div className="flex flex-wrap items-center justify-between gap-4 mt-auto pt-6">
                        <div className="flex flex-wrap items-center gap-3">
                            <span className="text-black font-black text-lg bg-yellow-200 px-2 py-1 border-2 border-black rounded shadow-[2px_2px_0px_0px_#000] whitespace-nowrap">
                                {contractor.price}
                            </span>
                            <span className={`flex items-center font-bold border-2 border-black px-2 py-1 rounded shadow-[2px_2px_0px_0px_#000] whitespace-nowrap ${contractor.available ? 'bg-green-200 text-black' : 'bg-gray-200 text-gray-600'}`}>
                                <span className={`w-3 h-3 rounded-full border-2 border-black ${contractor.available ? 'bg-green-500' : 'bg-gray-400'} mr-2 shrink-0`}></span>
                        ...
                        <div className="flex flex-wrap gap-3">
                            <Link
                                href={`/contractors/${contractor.id}`}
                                className="px-4 py-2 bg-white border-2 border-black text-black font-bold rounded-lg shadow-[3px_3px_0px_0px_#000] hover:translate-y-[-1px] hover:shadow-[4px_4px_0px_0px_#000] hover:bg-gray-50 transition-all text-center whitespace-nowrap"
                            >
                                View Profile
                            </Link>
                            <button
                                ...
                                className={`px-4 py-2 bg-blue-400 border-2 border-black text-black font-bold rounded-lg shadow-[3px_3px_0px_0px_#000] hover:translate-y-[-1px] hover:shadow-[4px_4px_0px_0px_#000] hover:bg-blue-500 transition-all text-center whitespace-nowrap ${!contractor.available ? 'opacity-50 cursor-not-allowed' : ''}`}
                            >
                                Book Now
                            </button>
                        </div>
