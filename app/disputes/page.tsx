...
export default function DisputesPage() {
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-12">
            ...
            <p className="text-black font-medium text-lg mb-8">Have a problem with a booking? Let us know and we&apos;ll resolve it within 24 hours.</p>

            {/* Dispute Form */}
            <form onSubmit={handleSubmit} className="bg-white border-4 border-black rounded-xl shadow-[8px_8px_0px_0px_#000] p-8 mb-12">
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label className="block text-black font-black mb-2 uppercase">Booking ID (Optional)</label>
                        <input
                            type="text"
                ...
                <div className="mb-6">
                    <label className="block text-black font-black mb-2 uppercase">Describe Your Issue *</label>
                    <textarea
            ...
            {loading ? (
                <div className="space-y-3">
                    {[...Array(3)].map((_, i) => (
                        <div key={i} className="bg-white border-3 border-black rounded-xl p-4 animate-pulse shadow-[3px_3px_0px_0px_#ccc]">
                ...
                <div className="space-y-3">
                    {disputes.map(dispute => (
                        <div key={dispute.id} className="bg-white border-3 border-black rounded-xl p-5 shadow-[4px_4px_0px_0px_#000]">
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                    <span className="text-lg">{statusIcon[dispute.status || 'In Review'] || '🔍'}</span>
                                    <span className="font-black capitalize">{dispute.type.replace(/([A-Z])/g, ' $1')}</span>
                                    {dispute.name && <span className="text-gray-500 font-medium">by {dispute.name}</span>}
                                </div>
                                <span className={`px-3 py-1 border-2 border-black rounded-lg font-bold text-xs ${statusColor[dispute.status || 'In Review'] || 'bg-gray-200'} shadow-[2px_2px_0px_0px_#000]`}>
                                    {dispute.status || 'In Review'}
                                </span>
                            </div>
                            <p className="text-black font-medium text-sm">{dispute.description}</p>
                            <p className="text-gray-400 text-xs font-bold mt-2">{new Date(dispute.created_at).toLocaleDateString('en-IN')}</p>
                        </div>
                    ))}
                </div>
            ) : (
