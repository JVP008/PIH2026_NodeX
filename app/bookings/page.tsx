        ...
        <div className="space-y-4">
          {filtered.length > 0 ? (
            filtered.map((booking) => (
              <div
                key={booking.id}
                className="bg-white border-3 border-black rounded-xl p-6 shadow-[4px_4px_0px_0px_#000] hover:translate-y-[-1px] hover:shadow-[5px_5px_0px_0px_#000] transition-all"
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-black text-lg">
                        {booking.contractor?.name || 'Unknown Contractor'}
                      </h3>
                  ...
                  <span
                    className={`px-3 py-1 border-2 border-black rounded-lg font-bold text-sm capitalize ${statusColor[booking.status || 'pending'] || 'bg-gray-200'} shadow-[2px_2px_0px_0px_#000]`}
                  >
                ...
                <div className="flex items-center justify-between border-t-2 border-dashed border-gray-300 pt-4">
                  ...
                  <div className="flex gap-2">
                    {booking.status === 'upcoming' && (
                      <>
                        <button
                          ...
                          className="px-4 py-2 bg-green-300 border-2 border-black rounded-lg font-bold shadow-[2px_2px_0px_0px_#000] hover:translate-y-[-1px] transition-all"
                        >
                          Pay ₹{booking.price}
                        </button>
                        <button
                          onClick={() => handleCancel(booking.id)}
                          className="px-4 py-2 bg-red-300 border-2 border-black rounded-lg font-bold shadow-[2px_2px_0px_0px_#000] hover:translate-y-[-1px] transition-all"
                        >
                          Cancel
                        </button>
                      </>
                    ...
                    {booking.status === 'completed' && (
                      <Link
                        href="/disputes"
                        className="px-4 py-2 bg-orange-300 border-2 border-black rounded-lg font-bold shadow-[2px_2px_0px_0px_#000] hover:translate-y-[-1px] transition-all"
                      >
                        Report Issue
                      </Link>
                    )}
